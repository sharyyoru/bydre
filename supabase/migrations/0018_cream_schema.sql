...
create index if not exists agents_last_name_idx on agents(last_name);

-- Agent insurance information
create table if not exists agent_insurances (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists agent_insurances_agent_id_idx on agent_insurances(agent_id);
...
create index if not exists deals_agent_id_idx on deals(agent_id);
...
create table if not exists crisalix_reconstructions (
  id uuid primary key default gen_random_uuid(),
  agent_id uuid not null references agents(id) on delete cascade,
  crisalix_agent_id integer not null,
  reconstruction_type text not null,
  player_id text,
  created_at timestamp with time zone default now() not null
);

create index if not exists crisalix_reconstructions_agent_type_idx on crisalix_reconstructions(agent_id, reconstruction_type);
...
create index if not exists emails_agent_id_idx on emails(agent_id);
...
create index if not exists whatsapp_messages_agent_id_idx on whatsapp_messages(agent_id);
...
-- Agent notes (internal collaboration notes on a patient)
...
create index if not exists agent_notes_agent_id_idx on agent_notes(agent_id);
...
-- Mentions/messages generated from Agent notes
create table if not exists agent_note_mentions (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists agent_note_mentions_recipient_idx
  on agent_note_mentions(mentioned_user_id, read_at);
...
-- Tasks linked to a Agent with optional assignee
...
-- Checklist items for tasks (Agent or project)
...
-- Agent edit locks (which user is currently editing which patient)
...
create index if not exists agent_edit_locks_user_id_idx
  on agent_edit_locks(user_id);
